...
const sessionService = require('../../../src/domain-providers/sessionService')
const messages = require('../../../src/message-catalog/messages')
...
class RegisterController {
  ...
  async getServiceTerms(req, res) {
    try {
      res.status(200).json({ title: '服务条款', content: '中国铁路客户服务中心网站服务条款内容...' })
    } catch (error) {
  ...
  async getPrivacyPolicy(req, res) {
    try {
      res.status(200).json({ title: '隐私权政策', englishTitle: 'NOTICE', content: '隐私权政策内容...' })
    } catch (error) {
