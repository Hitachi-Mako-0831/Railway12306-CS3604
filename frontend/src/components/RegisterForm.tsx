...
import { validateUsername as apiValidateUsername, validateIdCard as apiValidateIdCard } from '../api/register';
...
const RegisterForm: React.FC<RegisterFormProps> = ({ onSubmit }) => {
  ...
  const validateUsername = async (value: string) => {
    ...
    try {
      ...
      if (response.valid) {
        ...
        setUsernameValidation({ isValid: false, errorMessage: response.error, showCheckmark: false });
      }
    } catch (error: any) {
  ...
  const validateIdCard = async (value: string) => {
    ...
    // 调用API检查证件号是否已注册
    try {
      const response = await apiValidateIdCard(idCardType, value);
      if (response.valid) {
        setIdCardValidation({ isValid: true, errorMessage: '', showCheckmark: true });
      } else {
        setIdCardValidation({ isValid: false, errorMessage: response.error, showCheckmark: false });
      }
    } catch (error: any) {
      const errorMsg = error.response?.data?.error || '验证失败，请稍后重试';
