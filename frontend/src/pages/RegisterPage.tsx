...
const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (data: RegistrationData) => {
    ...
    try {
      ...
      // 步骤2: 发送验证码
      try {
        const verifyResponse = await sendRegistrationVerificationCode({ sessionId, phone: data.phone })
        
        // 从后端获取真实验证码（开发环境）
        const realCode = verifyResponse.verificationCode
        if (realCode) {
          console.log(`\n=================================`)
          console.log(`📱 注册验证码`)
  ...
  const handleVerificationComplete = async (code: string) => {
    ...
    try {
      ...
      // 调用后端完成注册API
      await completeRegistration({ sessionId, smsCode: code })
